                ...
                <div className="mt-8 flex flex-col gap-1">
                  ...
                  {navLinks.map(link => (
                    <Link key={link.to} to={link.to} onClick={() => setMobileOpen(false)}
                      className={`px-4 py-3 rounded-lg font-medium transition-colors ${isActive(link.to) ? 'bg-primary/10 text-primary' : 'text-foreground/70 hover:bg-muted'}`}>
                      {link.label}
                    </Link>
                  ))}
                    ...
                    <>
                      {isAdmin && (
                        <Link to="/admin" onClick={() => setMobileOpen(false)}
                          className="px-4 py-3 rounded-lg font-medium flex items-center gap-2 hover:bg-muted">
                          <Shield className="h-4 w-4" /> {t('nav.admin')}
                        </Link>
                      )}
                      ...
                      <button onClick={() => { logout(); setMobileOpen(false); }}
                        className="px-4 py-3 rounded-lg font-medium flex items-center gap-2 hover:bg-muted text-left">
                        <LogOut className="h-4 w-4" /> {t('nav.logout')}
                      </button>
                    </>
                  ) : (
                    <Link to="/login" onClick={() => setMobileOpen(false)}
                      className="px-4 py-3 rounded-lg font-medium text-primary hover:bg-muted">
                      {t('nav.login')}
                    </Link>
                  )}
                </div>
